...
import { SidebarProvider, Sidebar, SidebarContent, SidebarMenu, SidebarMenuItem, SidebarMenuButton, SidebarInset, SidebarHeader, SidebarFooter, SidebarGroup, SidebarGroupLabel, SidebarSeparator, SidebarTrigger, useSidebar, } from '@/components/ui/sidebar';
...
const ChatStateProvider = ({ children }) => {
    ...
    return (<ChatStateContext.Provider value={{
...
const ChatHistoryItem = ({ session, isSelected, onSelect, isGuest }) => {
        ...
        if (isGuest) {
            switch (action) {
                case 'rename':
                    ...
                    break;
                case 'archive':
                    setGuestSessions(prev => prev.map(s => s.id === session.id ? Object.assign(Object.assign({}, s), { isArchived: true }) : s));
                    toast({ title: "Archived", description: "Guest chat session archived." });
                    break;
                case 'unarchive':
                    setGuestSessions(prev => prev.map(s => s.id === session.id ? Object.assign(Object.assign({}, s), { isArchived: false }) : s));
    ...
    const submitRename = () => {
        if (isGuest) {
            setGuestSessions(prev => prev.map(s => s.id === session.id ? Object.assign(Object.assign({}, s), { title: newTitle.trim() }) : s));
            setIsRenaming(false);
            return;
        }
    ...
    return (<SidebarMenuItem className="group/item relative">
      {isRenaming ? (<div className="flex items-center gap-1 p-1 bg-accent/20 rounded-md">
          <Input value={newTitle} onChange={(e) => setNewTitle(e.target.value)} className="h-7 text-xs px-2 py-0" autoFocus onKeyDown={(e) => {
                if (e.key === 'Enter')
                    submitRename();
                if (e.key === 'Escape')
                    setIsRenaming(false);
          ...
          <SidebarMenuButton isActive={isSelected} onClick={() => onSelect(session.id)} className="pr-8">
          ...
          <div className="absolute right-1 top-1/2 -translate-y-1/2 opacity-0 group-hover/item:opacity-100 transition-opacity">
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-48">
                ...
                {session.isArchived ? (<DropdownMenuItem onClick={() => handleAction('unarchive')}>
                    <RotateCcw className="mr-2 h-4 w-4"/> Restore
                  </DropdownMenuItem>) : (<>
                    {!isGuest && (<DropdownMenuItem onClick={() => handleAction('pin')}>
...
const ChatSidebar = () => {
    ...
    const handleNewChat = async () => {
        ...
        try {
            const docRef = await addDoc(collection(firestore, 'users', user.uid, 'chatSessions'), newChatSession);
    ...
    const { data: cloudSessions, isLoading: sessionsLoading } = useCollection(chatSessionsQuery);
    const rawSessions = (user === null || user === void 0 ? void 0 : user.isAnonymous) ? guestSessions : cloudSessions || [];
    // Filtering logic including content for guest sessions
    const filteredSessions = rawSessions.filter(s => {
        var _a, _b;
        const q = searchQuery.toLowerCase();
        const titleMatch = (_a = s.title) === null || _a === void 0 ? void 0 : _a.toLowerCase().includes(q);
        // Content search for guests (as they are fully in memory)
        const contentMatch = (user === null || user === void 0 ? void 0 : user.isAnonymous) ? (_b = s.messages) === null || _b === void 0 ? void 0 : _b.some(m => m.content.toLowerCase().includes(q)) : false;
    ...
    const filteredGroups = Object.entries(regularSessions.reduce((acc, session) => {
        var _a;
        const date = ((_a = session.createdAt) === null || _a === void 0 ? void 0 : _a.seconds) ? new Date(session.createdAt.seconds * 1000) : new Date(session.createdAt);
    ...
    return (<>
      <SidebarHeader className="pt-4 px-2">
        <div className={cn("flex h-10 items-center mb-4", state === "expanded" ? "justify-between" : "justify-center")}>
            {state === "expanded" && (<div className="flex items-center overflow-hidden">
        ...
        {state === "expanded" && (<Button onClick={handleNewChat} className="w-full bg-primary hover:bg-primary/90 text-white shadow-md font-semibold">
      ...
      <SidebarContent className="scrollbar-thin scrollbar-visible">
          ...
          <SidebarMenu>
            <SidebarMenuItem>
              <div className="relative px-2 py-1">
                ...
                <Input placeholder="Search chats" className="pl-8 h-8 text-xs bg-muted/50 border-none focus-visible:ring-1 focus-visible:ring-primary" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}/>
              </div>
            </SidebarMenuItem>
          </SidebarMenu>
        </SidebarGroup>
        ...
        {archivedSessions.length > 0 && (<SidebarGroup>
            <SidebarGroupLabel>Archived</SidebarGroupLabel>
            <SidebarMenu>
              {archivedSessions.map(session => (<ChatHistoryItem key={session.id} session={session} isSelected={selectedChatId === session.id} onSelect={setSelectedChatId} isGuest={!!(user === null || user === void 0 ? void 0 : user.isAnonymous)}/>))}
            </SidebarMenu>
          </SidebarGroup>)}

        <SidebarSeparator />
        
        {sessionsLoading && (<SidebarGroup>
                 <SidebarGroupLabel>Loading history...</SidebarGroupLabel>
                 <div className="h-8 w-full bg-muted animate-pulse rounded-md mt-2"/>
                 <div className="h-8 w-full bg-muted animate-pulse rounded-md mt-2"/>
            </SidebarGroup>)}

        {!sessionsLoading && pinnedSessions.length > 0 && (<SidebarGroup>
            <SidebarGroupLabel>Pinned</SidebarGroupLabel>
            <SidebarMenu>
              {pinnedSessions.map(session => (<ChatHistoryItem key={session.id} session={session} isSelected={selectedChatId === session.id} onSelect={setSelectedChatId} isGuest={!!(user === null || user === void 0 ? void 0 : user.isAnonymous)}/>))}
            </SidebarMenu>
          </SidebarGroup>)}

        {!sessionsLoading && filteredGroups.map(([group, sessions]) => (<SidebarGroup key={group}>
                <SidebarGroupLabel>{group}</SidebarGroupLabel>
                <SidebarMenu>
                    {sessions.map((session) => (<ChatHistoryItem key={session.id} session={session} isSelected={selectedChatId === session.id} onSelect={setSelectedChatId} isGuest={!!(user === null || user === void 0 ? void 0 : user.isAnonymous)}/>))}
                </SidebarMenu>
            </SidebarGroup>))}
          ...
          <DropdownMenu>
              <DropdownMenuTrigger asChild>
                  <Button variant="ghost" className="w-full justify-start items-center gap-3 p-2 h-auto">
                      ...
                      {state === "expanded" && (<div className="flex flex-col items-start text-sm overflow-hidden text-left">
                              <span className="font-semibold truncate w-full">{(user === null || user === void 0 ? void 0 : user.displayName) || ((user === null || user === void 0 ? void 0 : user.isAnonymous) ? 'Guest' : 'User')}</span>
                              <span className="text-muted-foreground truncate w-full">{(user === null || user === void 0 ? void 0 : user.email) || ((user === null || user === void 0 ? void 0 : user.isAnonymous) ? 'Temporary session' : '')}</span>
                          </div>)}
                  </Button>
              </DropdownMenuTrigger>
...
const MainContentHeader = () => {
    ...
    const isGuest = selectedChatId === null || selectedChatId === void 0 ? void 0 : selectedChatId.startsWith('guest-');
    const chatSessionRef = useMemoFirebase(() => {
        if (!user || !selectedChatId || isGuest)
            return null;
        return doc(firestore, `users/${user.uid}/chatSessions/${selectedChatId}`);
    }, [firestore, user, selectedChatId, isGuest]);
    const { data: cloudChatSession } = useDoc(chatSessionRef);
    ...
    return (<header className="sticky top-0 z-10 flex items-center justify-between p-4 border-b bg-background/50 backdrop-blur-sm h-[64px]">
            ...
            <div className="flex items-center gap-2">
                <ThemeToggle />
                {selectedChatId && !isGuest && (<DropdownMenu>
                        <DropdownMenuTrigger asChild>
                        ...
                        <DropdownMenuContent align="end" className="w-48">
                            <DropdownMenuItem onClick={() => handleAction('pin')}>
                                <Pin className="mr-2 h-4 w-4"/> {(chatSession === null || chatSession === void 0 ? void 0 : chatSession.isPinned) ? 'Unpin Chat' : 'Pin Chat'}
                            </DropdownMenuItem>
                            <DropdownMenuItem onClick={() => handleAction('archive')}>
                                <Archive className="mr-2 h-4 w-4"/> Archive
                            </DropdownMenuItem>
...
const MainContentBody = () => {
    var _a;
    const { selectedChatId, setSelectedChatId, guestSessions, setGuestSessions, isGuestLoading, setIsGuestLoading } = useChatState();
    const { user } = useUser();
    ...
    const messagesQuery = useMemoFirebase(() => {
        if (!user || !selectedChatId || isGuest)
            return null;
        return query(collection(firestore, `users/${user.uid}/chatSessions/${selectedChatId}/chatMessages`), orderBy('timestamp', 'asc'));
    }, [firestore, user, selectedChatId, isGuest]);
    const { data: cloudMessages } = useCollection(messagesQuery);
    const guestMessages = isGuest ? (_a = guestSessions.find(s => s.id === selectedChatId)) === null || _a === void 0 ? void 0 : _a.messages : [];
    ...
    const handleWelcomeSubmit = async (e) => {
        ...
        if (!targetChatId) {
            if (user.isAnonymous) {
                ...
                const sessionDocRef = await addDoc(collection(firestore, 'users', user.uid, 'chatSessions'), newChatSession);
        ...
        if (user.isAnonymous) {
            ...
            setIsLoading(false);
            const userMessage = { senderType: "user", content: tempUserInput, timestamp: new Date() };
            setGuestSessions(prev => prev.map(s => s.id === targetChatId ? Object.assign(Object.assign({}, s), { messages: [...(s.messages || []), userMessage] }) : s));
            try {
                const { response } = await chatWithAi({ message: tempUserInput, mode: currentMode });
                const aiMessage = { senderType: "ai", content: response, timestamp: new Date() };
                setGuestSessions(prev => prev.map(s => s.id === targetChatId ? Object.assign(Object.assign({}, s), { messages: [...(s.messages || []), aiMessage] }) : s));
                generateChatTitle({ message: tempUserInput }).then(({ title }) => {
                    if (title) {
                        setGuestSessions(prev => prev.map(s => s.id === targetChatId ? Object.assign(Object.assign({}, s), { title }) : s));
                    }
                });
            }
            catch (err) {
    ...
    if (selectedChatId && !isEmpty) {
        ...
        return (<main className="flex-1 flex items-center justify-center p-4 md:p-6">
                <ChatInterface key={selectedChatId} chatSessionId={selectedChatId} initialInput={""} initialGuestMessages={currentGuestSession === null || currentGuestSession === void 0 ? void 0 : currentGuestSession.messages} onGuestMessagesChange={(updater) => {
                setGuestSessions(prev => prev.map(s => {
                    if (s.id === selectedChatId) {
                        const newMessages = typeof updater === 'function' ? updater(s.messages || []) : updater;
                        return Object.assign(Object.assign({}, s), { messages: newMessages });
                    }
                    return s;
                }));
            }} isGuestLoading={isGuestLoading} onGuestLoadingChange={setIsGuestLoading}/>
            </main>);
    }
    return (<main className="h-full flex flex-col justify-center items-center p-4 md:p-6 relative">
                ...
                <div className="relative group p-[1px] rounded-2xl bg-gradient-to-r from-primary/50 via-accent/30 to-primary/50 shadow-sm max-w-lg mx-auto">
                    <form onSubmit={handleWelcomeSubmit}>
                        <div className="relative flex items-center bg-background/80 backdrop-blur-md rounded-2xl overflow-hidden min-h-[40px]">
                            ...
                            <textarea value={input} onChange={(e) => setInput(e.target.value)} placeholder="Ask anything" className="block w-full border-0 bg-transparent py-2.5 pl-10 pr-4 text-sm ring-offset-background placeholder:text-muted-foreground/50 focus:outline-none focus:ring-0 transition-all resize-none overflow-hidden min-h-[40px]" rows={1} onKeyDown={(e) => {
